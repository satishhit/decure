...
class ProductAttributes extends \Magento\ImportExport\Model\Import\Entity\AbstractEntity
{
    ...
    protected function saveProductAttributes()
    {
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            foreach ($bunch as $rowNum => $rowData) {
                ...
                $this->processData($rowData, $attributeSetArr, $entityTypeId, $entityType);
            }
        }
    }

    /**
     * Process data to import
     * @param array $rowData
     * @param array $attributeSetArr
     * @param int $entityTypeId
     * @param string $entityType
     */
    protected function processData($rowData, $attributeSetArr, $entityTypeId, $entityType)
    {
        $attributeSetId = $this->getAttributeSetId($rowData, $attributeSetArr, $entityTypeId);
        ...
        if (empty($attributeId)) {
            ...
            if ($rowData['swatch_input_type']!="") {
                if (strpos($rowData['swatch_input_type'], 'text')!==false) {
                    $newData['optiontext'] = $this->getOptionDataValue($rowData, $attributeId);
                    $newData['swatchtext'] = $this->getOptionSwatchText($rowData, $entityType);
                    $newData['swatch_input_type'] = 'text';
                }

                if (strpos($rowData['swatch_input_type'], 'visual')!==false) {
                    $newData['optionvisual'] = $this->getOptionDataValue($rowData, $attributeId);
            ...
            $attributeResourceModel->setAttributeGroupId($newGroupId);
            $attributeResourceModel->setAttributeSetId($attributeSetId);
            ...
            if ($rowData['swatch_input_type']!="") {
                if (strpos($rowData['swatch_input_type'], 'text')!==false) {
                    $updateData['optiontext'] = $this->getOptionDataValue($rowData, $attributeId);
                    $updateData['swatchtext'] = $this->getOptionSwatchText($rowData, $entityType);
                    $updateData['swatch_input_type'] = 'text';
                }

                if (strpos($rowData['swatch_input_type'], 'visual')!==false) {
                    $updateData['optionvisual'] = $this->getOptionDataValue($rowData, $attributeId);
     ...
     * @return mixed|null
     */
    protected function getAttributeSetId($rowData, $attributeSetArr, $entityTypeId)
    {
        if ($rowData['attribute_set']=="") {
            return null;
        } elseif (!in_array($rowData['attribute_set'], $attributeSetArr)) {
            $this->attributeSet->setEntityTypeId($entityTypeId);
            $this->attributeSet->setAttributeSetName($rowData['attribute_set']);
            $this->attributeSet->save();
            return $this->attributeSet->getId();
        } else {
            $attributeSet = $this->attSetFactory->create()
                ->getCollection()
                ->addFieldToFilter('entity_type_id', $entityTypeId)
                ->addFieldToFilter('attribute_set_name', $rowData['attribute_set'])
                ->getFirstItem();

            return $attributeSet->getData('attribute_set_id');
        }
    }
                ...
                foreach ($optionAttributeValues as $optionAttributeValue) {
                    ...
                    if (isset($value[1])) {
                        if ($countValue<$countOptionId) {
                            ...
                            //$optionDataValues['order'][$optionId[$countValue]] = 0;
                        } else {
